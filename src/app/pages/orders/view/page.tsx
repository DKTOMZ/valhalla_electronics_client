...
const ViewOrder: React.FC = () => {
    ...
    useEffect(()=>{
        const fetchData = async() => {
            return await http.get<OrderType>(`${process.env.NEXT_PUBLIC_VALHALLA_URL}/api/orders/fetch/id?id=${orderId}`);
        };
            ...
            {order ? <form onSubmit={(e)=>{handleDispatch(e)}} className="flex flex-col gap-4 xl:w-2/3 2xl:w-1/2 w-full mx-auto">
                ...
                <div className="flex mb-4 pb-4 pt-4 text-black dark:text-white flex-col w-full border rounded-md dark:border-gray-400 border-zinc-800 px-4 overflow-hidden">
                    ...
                    <div className="mx-2">
                        ...
                        <div className={commonFieldsStyle}>
                            <div className={commonKeyStyle}>Delivery Status:</div>
                            <div className={`${commonValueStyle} ${order.deliveryStatus?.toUpperCase() == DeliveryStatus.DELIVERED ? '!text-green-500' : order.deliveryStatus?.toUpperCase() == DeliveryStatus.PENDING ? '!text-red-500': '!text-orange-500'}`}>{order.deliveryStatus?.toUpperCase()??''}</div>
                        </div>
                        <div className={commonFieldsStyle}>
                    ...
                    <FormSubmitButton disabled={loadingSave} text={loadingSave ? 'Printing Receipt' : 'Print receipt'} className="!ml-auto !mt-4 !w-fit !p-5"/>
                </div>
            </form> : null}
        </Layout>
    );
